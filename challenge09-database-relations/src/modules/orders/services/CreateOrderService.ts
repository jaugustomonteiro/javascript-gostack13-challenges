...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) throw new AppError('This customer not exists');

    const existentProducts = await this.productsRepository.findAllById(
      products,
    );

    if (!existentProducts.length) throw new AppError('This products not found');

    const existentProductsId = existentProducts.map(product => product.id);

    const ckectInexistentProduct = products.filter(
      product => !existentProductsId.includes(product.id),
    );

    if (ckectInexistentProduct.length)
      throw new AppError(
        `Cold not find product ${ckectInexistentProduct[0].id}`,
      );
